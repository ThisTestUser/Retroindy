...
public class Retroindy
{
	public static void main(String[] args) throws Throwable
	{
		...
		outName = outName.substring(0, outName.length() - ".jar".length())
			+ "-retro.jar";
		...
		try(ZipOutputStream outstream =
			new ZipOutputStream(new FileOutputStream(out));
			ZipFile zipFile = new ZipFile(in))
		{
			...
			while(enumeration.hasMoreElements())
			{
				...
				if(!next.isDirectory())
				{
					...
					{
						byte[] classBytes =
							IOUtils.toByteArray(zipFile.getInputStream(next));
	 ...
	 * </blockquote>
	 * </p>
	 * It also creates a helper function {@code asMethodType} with the following
	 * content
	 * <p>
	 * <blockquote>
	 * 
	 ...
	 * {
	 * 	return MethodType.fromMethodDescriptorString(arg0,
	...
	private static void handleLdcMethodType(ClassNode classNode)
	{
		...
		String asMethodTypeDesc =
			"(Ljava/lang/String;)Ljava/lang/invoke/MethodType;";
		{
			Set<String> blacklisted = new HashSet<>();
			for(MethodNode methodNode : classNode.methods)
				if(methodNode.desc.equals(asMethodTypeDesc)
					&& methodNode.name.startsWith(asMethodTypeName))
		...
		MethodNode asMethodType = new MethodNode(Opcodes.ASM5,
			Opcodes.ACC_STATIC, asMethodTypeName, asMethodTypeDesc, null, null);
		...
		asMethodTypeInstructions
			.add(new LdcInsnNode(Type.getObjectType(classNode.name)));
		asMethodTypeInstructions
			.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "java/lang/Class",
				"getClassLoader", "()Ljava/lang/ClassLoader;", false));
		asMethodTypeInstructions.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
		...
		for(MethodNode methodNode : classNode.methods)
		{
			...
			for(AbstractInsnNode now =
				methodNode.instructions.getFirst(); now != null; now =
					now.getNext())
					...
					if(ldc.cst instanceof Type)
					{
						...
						if(type.getSort() == Type.METHOD)
						{
							...
							replacement.add(new LdcInsnNode(
								((Type)ldc.cst).getDescriptor()));
							replacement.add(new MethodInsnNode(
								Opcodes.INVOKESTATIC, classNode.name,
								asMethodTypeName, asMethodTypeDesc, false));
	 ...
	 * {@link Retroindy#handleLdcMethodType(ClassNode)} so MethodTypes are
	 * converted properly
	 */
	...
	private static void handleType(String desc, InsnList list)
	{
		...
		if(type.getSort() == Type.BOOLEAN)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Boolean",
				"TYPE", "Ljava/lang/Class;"));
		else if(type.getSort() == Type.CHAR)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Character",
				"TYPE", "Ljava/lang/Class;"));
		else if(type.getSort() == Type.BYTE)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Byte",
				"TYPE", "Ljava/lang/Class;"));
		else if(type.getSort() == Type.SHORT)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Short",
				"TYPE", "Ljava/lang/Class;"));
		else if(type.getSort() == Type.INT)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Integer",
				"TYPE", "Ljava/lang/Class;"));
		else if(type.getSort() == Type.FLOAT)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Float",
				"TYPE", "Ljava/lang/Class;"));
		else if(type.getSort() == Type.LONG)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Long",
				"TYPE", "Ljava/lang/Class;"));
		else if(type.getSort() == Type.DOUBLE)
			list.add(new FieldInsnNode(Opcodes.GETSTATIC, "java/lang/Double",
				"TYPE", "Ljava/lang/Class;"));
	...
	/**
	 * This one converts {@code invokedynamic} instructions into regular
	 * invocation instructions with the help of
	 * additional methods and fields
	 * <p>
	 * JLS notes:
	 * Resolve MethodHandle, MethodType from bsm, static bsmArgs
	 * Simulate a invokevirtual (!) on java/lang/invoke/MethodHandle invoke
	 * (Ljava/lang/invoke/MethodHandle;Ljava/lang/invoke/MethodHandle$Lookup;Ljava/lang/String;Ljava/lang/invoke/MethodType;<other
	 * args>)Ljava/lang/invoke/CallSite;
	 * No boxing
	 * Descriptor is arbitrary (could return Object)
	 * If bootstrap is varargs, collect trailing args into varargs array
	 * If bootstrap is invoked concurrently, all are allowed to complete, but
	 * only one result is used
	 * todo:
	 * Handle varargs
	 * Linking/Runtime exceptions
	 * <p>
	 * Note that when the JVM bootstraps an invokedynamic instruction, the call
	...
	private static void handleIndy(ClassNode classNode)
	{
		...
		String callsiteArrayDesc =
			"Ljava/util/concurrent/atomic/AtomicReferenceArray;";
		{
			Set<String> blacklisted = new HashSet<>();
			for(FieldNode fieldNode : classNode.fields)
				if(fieldNode.desc.equals(callsiteArrayDesc)
					&& fieldNode.name.startsWith(callsiteArrayName))
		...
		FieldNode callsiteArray =
			new FieldNode(Opcodes.ASM5, Opcodes.ACC_STATIC, callsiteArrayName,
				callsiteArrayDesc, null, null);
		...
		for(MethodNode methodNode : classNode.methods)
		{
			...
			for(AbstractInsnNode now =
				...
				if(now instanceof InvokeDynamicInsnNode)
				{
					...
					MethodNode bootstrap = new MethodNode(Opcodes.ASM5,
						Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC,
						"INDY_" + thisId, indy.desc, null, null);
					...
					insns.add(
						new FieldInsnNode(Opcodes.GETSTATIC, classNode.name,
							callsiteArray.name, callsiteArray.desc));
					insns.add(new LdcInsnNode(thisId));
					insns.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL,
						"java/util/concurrent/atomic/AtomicReferenceArray",
						"get", "(I)Ljava/lang/Object;", false));
					insns.add(
						new JumpInsnNode(Opcodes.IFNONNULL, jumpIfCallsite));
					...
					insns.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
						indy.bsm.getOwner(), indy.bsm.getName(),
						indy.bsm.getDesc(), false));
					insns.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL,
						"java/util/concurrent/atomic/AtomicReferenceArray",
						"compareAndSet",
					...
					Type[] types =
						Type.getMethodType(bootstrap.desc).getArgumentTypes();
					...
					writer.visit(Opcodes.ASM5, 0, "a", null, "java/lang/Object",
						null);
					...
					InsnList replacement = new InsnList();
					replacement.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
						classNode.name, bootstrap.name, bootstrap.desc, false));
		...
		if(invokeDynamicCount > 0)
		{
			...
			if(clinit == null)
			{
				clinit = new MethodNode(Opcodes.ASM5, Opcodes.ACC_STATIC,
					"<clinit>", "()V", null, null);
			...
			initFields.add(new TypeInsnNode(Opcodes.NEW,
				"java/util/concurrent/atomic/AtomicReferenceArray"));
			...
			initFields.add(new MethodInsnNode(Opcodes.INVOKESPECIAL,
				"java/util/concurrent/atomic/AtomicReferenceArray", "<init>",
				"([Ljava/lang/Object;)V", false));
			initFields.add(new FieldInsnNode(Opcodes.PUTSTATIC, classNode.name,
				callsiteArray.name, callsiteArray.desc));
